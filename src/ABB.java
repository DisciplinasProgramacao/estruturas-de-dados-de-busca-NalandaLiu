...
public class ABB<K, V> implements IMapeamento<K, V>{
    ...
    public Lista<V> recortar(K chaveDeOnde, K chaveAteOnde) {
    	...
    	recortarRecursivo(raiz, chaveDeOnde, chaveAteOnde, resultado);
		return resultado;
	}
    
    /**
     * Método recursivo que percorre a árvore em ordem e adiciona à lista os elementos
     * cuja chave está no intervalo [chaveDeOnde, chaveAteOnde], inclusive.
     * @param raizArvore a raiz da árvore ou sub-árvore a ser percorrida.
     * @param chaveDeOnde a chave inicial do intervalo (inclusive).
     * @param chaveAteOnde a chave final do intervalo (inclusive).
     * @param resultado a lista que armazenará os elementos no intervalo.
     */
    private void recortarRecursivo(No<K, V> raizArvore, K chaveDeOnde, K chaveAteOnde, Lista<V> resultado) {
    	...
    	// Percorre a sub-árvore esquerda se a chave atual for maior que chaveDeOnde
    	if (comparacaoInicio > 0)
    		recortarRecursivo(raizArvore.getEsquerda(), chaveDeOnde, chaveAteOnde, resultado);
    	
    	// Se a chave atual está no intervalo [chaveDeOnde, chaveAteOnde], adiciona à lista
    	if (comparacaoInicio >= 0 && comparacaoFim <= 0)
    		resultado.inserir(raizArvore.getItem());
    	
    	// Percorre a sub-árvore direita se a chave atual for menor que chaveAteOnde
    	if (comparacaoFim < 0)
    		recortarRecursivo(raizArvore.getDireita(), chaveDeOnde, chaveAteOnde, resultado);
    }
